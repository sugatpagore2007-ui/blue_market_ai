...
- Autopilot runs in the background and keeps the terminal free.
- Terminal shows only important summaries, order-punched cards, manager actions, and errors.
- New trades display immediately after execution.
- Trade manager stays active in background after entry.
- Voice auto-start remains OFF.



## Phase 17 Clean Architecture
Run from project root with: `python blue.py`. Existing engine code remains here for compatibility.
