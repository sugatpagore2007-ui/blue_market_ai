...
# Phase 16.3: keep autopilot background and terminal clean.
# Only print start/stop, important cycle summaries, order-punched cards, and errors.
AUTOPILOT_CLEAN_TERMINAL = True
AUTOPILOT_PRINT_ONLY_IMPORTANT = True
...
def _important_cycle_card(cycle: int, analyzed: List[Dict[str, Any]], candidates: List[Dict[str, Any]], attempted: int, sent: int) -> str:
    """Short readable card. Avoid messy per-pair terminal spam."""
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    top = sorted([r for r in analyzed if not r.get('error')], key=_candidate_rank, reverse=True)[:3]
    lines = [
        "",
        "=" * 72,
        f"AUTOPILOT BACKGROUND SCAN #{cycle} | {now}",
        "=" * 72,
        f"Scanned      : {len(analyzed)} symbols",
        f"Eligible     : {len(candidates)} A/A+ setups",
    ...
    if top:
        ...
        for r in top:
            grade = _overall_grade(r) or 'N/A'
            direction = _signal_direction(r)
            status = 'READY' if r in candidates else 'WAIT/BLOCK'
            reason = _block_reason(r) if status != 'READY' else ''
            small_reason = (" | " + reason[:85]) if reason else ''
            lines.append(f"{r.get('symbol'):<10} {direction:<4} conf {str(r.get('confidence')):>5}% | grade {grade:<2} | {status}{small_reason}")
    if sent == 0:
        lines.append("-" * 72)
        lines.append("No new order. Terminal is free — type commands anytime.")
    lines.append("=" * 72)
    return "\n".join(lines)


def _manager_output_is_important(msg: str) -> bool:
    t = str(msg or '').lower()
    if not t.strip():
        return False
    important_words = [
        'moved', 'breakeven', 'partial', 'closed', 'trail', 'tp', 'sl', 'error',
        'failed', 'modified', 'position', 'profit', 'loss'
    ]
    # Hide repetitive status lines like "no positions".
    if 'no open' in t or 'nothing to manage' in t or 'no blue positions' in t:
        return False
...
def _autopilot_loop(scan_seconds: int = DEFAULT_SCAN_SECONDS, cycles: int = DEFAULT_CYCLES) -> None:
    ...
    if AUTOPILOT_CLEAN_TERMINAL:
        print("\n" + "=" * 72, flush=True)
        print("BLUE AUTOPILOT ACTIVE IN BACKGROUND", flush=True)
        print("=" * 72, flush=True)
        print("Scan interval : 5 minutes", flush=True)
        print("Terminal      : free for commands", flush=True)
        print("Manager       : always active after entry", flush=True)
        print("Stop command  : blue autopilot off", flush=True)
        print("=" * 72, flush=True)
    else:
    ...
    try:
        mgr_start = start_auto_manager_background()
        if not AUTOPILOT_CLEAN_TERMINAL:
            print(mgr_start, flush=True)
    except Exception as exc:
    ...
    try:
        for cycle in range(1, int(cycles) + 1):
            ...
            if AUTOPILOT_ONLY_BEST_TRADE:
                ...
                for symbol in AUTOPILOT_SYMBOLS:
                    ...
                    try:
                        ...
                        if not AUTOPILOT_CLEAN_TERMINAL:
                            print(line, flush=True)
                        if (not AUTOPILOT_CLEAN_TERMINAL) and AUTOPILOT_SHOW_BASIS_FOR_ALL_SIGNALS and _signal_direction(result) in ['BUY', 'SELL']:
                ...
                for result in candidates:
                    ...
                    if (not AUTOPILOT_CLEAN_TERMINAL) and AUTOPILOT_SHOW_TRADE_BASIS:
                        basis = _autopilot_trade_basis(result, mode='candidate')
                        print(basis, flush=True)
                        cycle_lines.append(basis)
                    exec_msg = execute_signal_if_allowed(result)
                    if (not AUTOPILOT_CLEAN_TERMINAL) or ("AUTO TRADE DONE" not in str(exec_msg)):
                        print(exec_msg, flush=True)
                ...
                if AUTOPILOT_CLEAN_TERMINAL:
                    print(_important_cycle_card(cycle, analyzed, candidates, attempted_orders, successful_orders), flush=True)
            ...
            try:
                ...
                if (not AUTOPILOT_CLEAN_TERMINAL) or _manager_output_is_important(manager_msg):
        ...
        if AUTOPILOT_CLEAN_TERMINAL:
            print("\nBlue autopilot stopped. Terminal is free.", flush=True)
        else:
